...
def probExtractor(drug_id, edge_index, rel_index, shortest_paths, num_rel, fixed_num, pagerank_path):
    ...
    for d in drug_id:
        ...
        out_nei = list(g.successors(d_int))
        k = fixed_num 
        ...
        if len(out_nei) == 0:
            ...
            p = np.full(N, tele, dtype=np.float64)

            weights = np.array([g[d_int][v].get("weight", 1.0) for v in out_nei], dtype=np.float64)
            wsum = weights.sum()

            if wsum == 0:
                p[:] = 1.0 / N
            else:
                for v, w in zip(out_nei, weights):
                    p[node2idx[v]] += alpha * (w / wsum)

        neighbors = np.random.choice(nodes, size=k, replace=False, p=p).tolist()
        subsets.extend([int(x) for x in neighbors])
        seen = set()
        subsets = [x for x in subsets if (x not in seen and not seen.add(x))]
