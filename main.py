...
def init_args(user_args=None):
    ...
    parser = argparse.ArgumentParser(description='SRKGE')

    parser.add_argument('--model_name', type=str, default='SRKGE')
    ...
    parser.add_argument('--extractor', type=str, default="probability") ##option [khop-subtree, randomWalk, probability]
...
def load_data(args):
    ...
    print("load networks !!")
    num_node, network_edge_index, network_rel_index, num_rel = read_network(data_path + "kg_extended 14 sim 0.88 score 0.92 topk 20 80000.txt")
...
def init_model(args, dataset_statistics):
    if args.model_name == 'SRKGE':
        model = SRKGE(max_layer=args.layer,
    ...
    for fold, (train_idx, test_idx, val_idx) in enumerate(zip(*split_fold(k_fold, data, labels, args.s_type))):
        ...
        if args.model_name:
            ...
            save_dir = os.path.join('./best_save/', args.model_name, args.dataset, args.extractor,
                                    "fold_{}".format(fold), "{:.5f} kg_extended 14 sim 0.88 score 0.92 topk 20 80000".format(test_log['auc']))
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            model.save(save_dir)
